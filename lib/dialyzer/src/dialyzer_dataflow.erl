...
contract_return_type({M, cast, _} = MFA, C, #state{callgraph = Callgraph}) when M =:= 'gen_server'; M =:= 'Elixir.GenServer' ->
...
contract_return_type({M, call, _} = MFA, C, #state{callgraph = Callgraph}) when M =:= 'gen_server'; M =:= 'Elixir.GenServer' ->
  case dialyzer_callgraph:get_gen_server_detection(Callgraph) of
    true -> fun(FunArgs) ->
      R = case FunArgs of
...
contract_return_type(MFA, C, _State) ->
  contract_return_type1(MFA, C).
contract_return_type1(_, C) ->
  fun(FunArgs) ->
...
handle_apply_or_call([{TypeOfApply, {Fun, Sig, Contr, LocalRet}}|Left],
  ...
  {CArgs, CRange} =
	...
	{As, contract_return_type(Fun, C, State)};
      none -> GenSig
    end,
  {BifArgs, BifRange} =
    case TypeOfApply of
  ...
  case any_none([MType, FType|As]) of
	...
	true ->
	  ...
	  case {t_atom_vals(MType), t_atom_vals(FType)} of
	    {[MAtom], [FAtom]} ->
	      FunInfo = [{remote, state__fun_info({MAtom, FAtom, length(Args)}, As,
						  State1)}],
    ...
    true ->
      ...
      LookupType = case LookupTypeTemp of
                     none ->
                       ...
                       ?log("[DATAFLOW(~p)]: Plt lookup for: ~n~p~n~n", [_UniqueId, HandleCallMFA], State),
                     ...
                     T ->
                       ...
                       ?log("[DATAFLOW(~p)]: Lookup type found for Plt lookup with arity~n~n", [_UniqueId], State),
                       T
                   end,

      ?log("[DATAFLOW(~p)]: Final result of Plt lookup: ~n~p~n~n", [_UniqueId, LookupType], State),

      case LookupType of
        none ->
          ?log("[DATAFLOW(~p)]: Fallback to Dialyzer lookup: any()~n~n", [_UniqueId], State),
...
state__fun_info(MFA, _As, State) ->
  state__fun_info1(MFA, State).

state__fun_info1({_, _, _} = MFA, #state{plt = PLT}) ->
  {MFA,
    dialyzer_plt:lookup(PLT, MFA),
    dialyzer_plt:lookup_contract(PLT, MFA),
    t_any()}.
